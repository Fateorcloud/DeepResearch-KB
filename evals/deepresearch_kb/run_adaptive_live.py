...
async def main(query, database, kb, output):
    ...
    (folder/"route.json").write_text(json.dumps({"query":query,"final_route":route,"evidence_count":len(evidence),"quick_calls":len(quick_calls),"deep_calls":len(deep_calls),"decisions":[d.__dict__ for d in decisions],"latency_seconds":time.perf_counter()-started},indent=2))
    print(folder)

if __name__=="__main__":
    p=argparse.ArgumentParser(); p.add_argument("query"); p.add_argument("--database",required=True); p.add_argument("--kb",required=True); p.add_argument("--output",required=True); a=p.parse_args(); asyncio.run(main(a.query,a.database,a.kb,a.output))
