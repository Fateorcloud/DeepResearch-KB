...
async def main(query, database, kb, output, max_deep_calls=1, requirements_path=None):
    ...
    if requirements_path:
        ...
        requirement = EvidenceRequirement(
            requirements["id"], tuple(requirements["required_claims"]),
            tuple(requirements.get("required_source_types", [])),
            int(requirements.get("minimum_distinct_sources", 1)),
            bool(requirements.get("require_current_version", False)),
        )
    else:
    ...
    (folder/"route.json").write_text(json.dumps({"query":query,"requirement":requirement.__dict__,"final_route":route,"evidence_count":len(evidence),"quick_calls":len(quick_calls),"deep_calls":len(deep_calls),"max_deep_calls":max_deep_calls,"decisions":[d.__dict__ for d in decisions],"latency_seconds":time.perf_counter()-started},indent=2))
    print(folder)

if __name__=="__main__":
    p=argparse.ArgumentParser(); p.add_argument("query"); p.add_argument("--database",required=True); p.add_argument("--kb",required=True); p.add_argument("--output",required=True); p.add_argument("--max-deep-calls",type=int,default=1); p.add_argument("--requirements"); a=p.parse_args(); asyncio.run(main(a.query,a.database,a.kb,a.output,a.max_deep_calls,a.requirements))
